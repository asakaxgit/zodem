...
- [x] A lint script / ESLint config — root-level flat `eslint.config.mjs`
      (`typescript-eslint` recommended, non-type-aware since package tsconfigs only include
      `src`), `pnpm lint` / `pnpm lint:fix`, wired into CI
