...
const relativeName = (fullName: string, packageName: string): string => {
  return fullName.startsWith(`${packageName}.`) ? fullName.slice(packageName.length + 1) : fullName;
};

const typeNameFor = (type: IRType, packageName: string): string => {
  switch (type.kind) {
    case "scalar":
      return type.name;
    case "message":
...
const emitReserved = (reserved: IRReserved[], indent: string): string[] => {
...
const renderRuleValue = (value: IRRuleValue): string => {
...
const renderRuleSetWrapped = (rules: IRRuleSet): string => {
  return `${rules.group}: {${renderRuleSetBody(rules)}}`;
};

const renderRuleSetBody = (rules: IRRuleSet): string => {
...
const emitFieldLine = (field: IRField, packageName: string, indent: string, validate: boolean): string => {
...
const emitMessage = (msg: IRMessage, packageName: string, indentLevel: number, validate: boolean): string[] => {
...
const emitEnum = (en: IREnum, indentLevel: number): string[] => {
  const indent = "  ".repeat(indentLevel);
  const inner = "  ".repeat(indentLevel + 1);
...
const emitService = (svc: IRService, packageName: string): string[] => {
...
export const emitProto = (input: EmitFileInput): string => {
...
const messageHasRules = (msg: IRMessage): boolean => {
 ...
 */
export const computeFileImports = (
