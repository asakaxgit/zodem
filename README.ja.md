...
|---|---|---|---|---|---|
| 何を書くか | Zod スキーマ | TSの型/プロシージャ（暗黙的に共有） | `.proto` ファイル | Zod スキーマ | Zod スキーマ、`.proto`、グルーコード — 三箇所に分かれて |
| 何が生成されるか | Zod スキーマから`.proto`とランタイムコーデックを生成 | 何も生成されない — クライアントがサーバーの型を直接importする | `.proto`からTSの型/クライアントを生成 | Zod スキーマからOpenAPI ドキュメント/クライアントを生成 | 何も生成されない — すべてを記憶で同期させる |
| ワイヤーフォーマット | Protobuf（バイナリ） | なし — 通常HTTP+JSON経由の関数呼び出し | Protobuf（バイナリ） | JSON（OpenAPI） | 大昔に決めた何か |
| 多言語クライアント | ✅ protobuf | ❌ TSのみ — クライアントとサーバーが同じTSの型を直接共有する必要がある | ✅ protobuf | ✅ OpenAPI コード生成経由 | 🤷 誰かが言語ごとに何度も再実装したもの |
| リネームでフィールド番号は保持されるか | ✅ `zodem rename` + ロックファイル | N/A — ワイヤースキーマがない | ⚠️ 自分でフィールド番号を管理・固定する必要がある | N/A — フィールド番号という概念がない | ❌ 誰も見ていない |
| バリデーションルールはスキーマと一緒に運ばれるか | ✅ 同じZod チェック → `buf.validate` | N/A — バリデーションはただのZod（プロセス内） | ⚠️ `buf.validate`を別途手書きする、またはスキップする | ✅ 同じZod チェック → OpenAPIの制約 | ❌ 各側で別々に再定義され、静かにズレていく |
...
セットアップ、ビルド順序、PRの規約については[CONTRIBUTING.md](CONTRIBUTING.md)（英語）を、コードスタイルのルールとその理由については[CONVENTIONS.md](CONVENTIONS.md)（英語）を、バージョニングとnpm公開の仕組みについては[RELEASING.md](RELEASING.md)（英語）を参照してください。
